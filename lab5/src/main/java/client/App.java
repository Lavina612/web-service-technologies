...
public class App {
    private static final String URL = "http://localhost:9050/rest/persons";
    ...
    private static void getPersons(Client client, Scanner scanner) {
        ...
        String position = getParam(scanner, "Position");
    ...
    private static void addPerson(Client client, Scanner scanner) {
        ...
        ClientResponse response = webResource.type(MediaType.APPLICATION_JSON).post(ClientResponse.class, person);
        if (response.getStatus() != ClientResponse.Status.OK.getStatusCode()) {
            throw new IllegalStateException("Request failed");
        }
    ...
    private static void updatePerson(Client client, Scanner scanner) {
        ...
        ClientResponse response = webResource.type(MediaType.APPLICATION_JSON).put(ClientResponse.class, person);
        if (response.getStatus() != ClientResponse.Status.OK.getStatusCode()) {
            throw new IllegalStateException("Request failed");
        }
